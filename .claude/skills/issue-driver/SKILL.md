...
### エスカレーション ＝「あなたが止まっている」signal

`escalation` は「人にやってほしいことが残っている」ではなく、**あなたの自走が実際に止まっていて、人間の入力なしには次の一歩も踏めない**ことを表す。cockpit はこれを「今すぐ見に行くべき run」として赤で描くので、**走り続けているのに立てたままだと、赤が「見に行っても普通に動いている」を意味するようになり、レンズの最も強いシグナルが死ぬ**。

- **走れるなら立てない**：あるサブタスクが認証・権限待ちで越えられなくても、**他のサブタスクを進められるなら run 全体は止まっていない**。そのノードを `liveStatus: "blocked"` にし、人にやってほしいことを `activity` に一行で書いて、**残りを進める**。run レベルの `escalation` は立てない。
- **自走を再開したら降ろす**：発生と同じくらい「解消」が重要。自分の推奨を自分で採用して先へ進むなら、**進み始めるその時点で `escalation` を `null` に戻す**。人への依頼が残っていることは、ブロックされたノードの `blocked` + `activity` が表す。**立てたまま走り続けてはいけない**。
- **立てる時は完全に止まる**：3条件を満たし、かつ**本当に一歩も進めない**時だけ `escalation` を立て、同時に `phase: "escalated"` にして人間を待つ。逆に言えば `escalation != null` と `phase: "escalated"` は常に一致する。
...
- **詰まった**：3条件でエスカレーション。ただし**止まった時だけ**——他に進められる仕事が残っているなら、そのノードを `blocked` にして先へ進む（§エスカレーション ＝「あなたが止まっている」signal）。
- **人待ちが解けた／回り道が見つかった**：`escalation` を `null` に戻して `phase` を実作業のフェーズへ戻す。降ろし忘れると cockpit が赤を出し続ける。
...
**更新契機**：フェーズ遷移・各ノードの状態変化・エスカレーションの発生/解消のたび。**解消の書き忘れは発生の書き忘れと同じ重さの事故**（§エスカレーション ＝「あなたが止まっている」signal）——自走を再開したのに `escalation` が残っていると、cockpit は止まっていない run を赤で描き続ける。

**時刻は実測値を書く**：`updatedAt` と `escalation.at` は `date -u +%Y-%m-%dT%H:%M:%SZ` で**その場で取得**する。JST から手で引き算して書かない——未来時刻を書くと cockpit 側の鮮度判断が壊れる。
...
```jsonc
{
  ...
  "updatedAt": "<ISO8601>",
  "escalation": null,   // 自走が止まっている時だけ非 null（その時は phase も "escalated"）。走れるなら null のまま——止まっていない run を赤くしない
                        // 非 null の形: { "reason": "spec_conflict|external_impact|blocker", "detail": "…", "options": ["…"], "recommendation": "…", "at": "<ISO8601>" }
  "nodes": [
    {
      "key": "t1",
      "title": "…",
      ...
      "activity": "実装中: xxx を追加",   // 人が読む一行(任意)。人にやってほしいことがあればここに一行で
      "subIssue": 71,                      // GitHub 参照(なければ null)
      "prNumber": 77,                      // GitHub 参照(なければ null)。マージ状態は書かない
      "escalation": null                   // run と同じ規律：このノードで止まって人待ちの時だけ非 null。
                                           // 他を進められるなら liveStatus: "blocked" + activity で表す
    }
  ]
}
```
...
- 自走を続けられるのに `escalation` を立てる／立てたまま走り続ける（cockpit の赤が「見に行っても動いている」を意味するようになり、レンズが信用を失う）
- 進捗ファイルの時刻を手計算で書く（未来時刻になり鮮度判断が壊れる。`date -u` で実測する）
