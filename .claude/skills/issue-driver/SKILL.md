...
**1周の reconcile で必ず取得する4点セット（省略不可）**：reconcile は「GitHub を眺める」ではなく、下の4つを**毎周まとめて取得する**ことと定義する。1つでも取り逃すと反応漏れになる。
1. **PR のマージ状態**（マージ検知の起点）。
2. **CI チェックのステータス**（`gh pr checks <PR>` 等。green か落ちか）。
3. **新規レビューコメント（inline）**：`gh api repos/OWNER/REPO/pulls/<PR>/comments`。Claude PR Review bot 等が付ける行単位の `[must]` 指摘はここにしか出ず、`gh pr checks` には現れない。
4. **新規会話コメント（issue コメント）**：`gh api repos/OWNER/REPO/issues/<PR>/comments`（bot の PR summary 等のノイズは除外可）。

> **red flag**：**CI ステータス（`gh pr checks`）だけを見張る監視は、3・4 のレビューコメントを黙って取りこぼす。** CI が green でも `[must]` 指摘は残る。毎周 3・4 まで取得して初めて reconcile 1周とみなす。3・4 は `created_at` 等で前周以降の新規分だけ拾えば二重反応を避けられる。
