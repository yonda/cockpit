...
**回し方（基盤非依存）**：ループは「一定間隔で GitHub を reconcile する」とだけ規定する。実際の起床（監視ツール／スケジュール起床／マージ契機での再 kick）は環境が提供する手段に委ねる。**ホットループでポーリングし続けない**（トークンを浪費する）。かつ **stateless-recoverable**：kill されても再 kick 時に GitHub＋進捗ファイルから状態を再構築してループを再開できること（§stateless-recoverable）。マージは絶対にしない——検知して反応するだけ。
