...
3. **解けないブロッカー**：権限・認証・環境・依存の欠落など、自力で越えられない障害
...
- 依存の無いサブタスクから **teammate を立てて**割り当てる（§teammates）。lead（あなた）は統合とレビューを担う。
...
PR を出す前に、自分の変更を次の観点で**自分で**通し、指摘をトリアージする：
...
- 各 teammate には担当サブタスク（対応する sub-issue）を渡し、「実装 → セルフレビュー → draft PR → lead に報告」させる。**teammate は自己終了せず lead に結果（PR URL・サマリ）を報告**する。
