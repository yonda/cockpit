...
// ドロワー本体は body 直下に portal で出す。ヘッダは sticky + backdrop-blur で
// 独自の stacking context を持つため、中に置くと高さと z-index がヘッダに閉じ込められる。
export function NavDrawer() {
  const pathname = usePathname();
  // 「開いた時点の pathname」を持ち、現在の pathname と一致する間だけ開いているとみなす。
  // Link クリックでも数字キーでも、遷移すれば effect なしで閉じる。
  const [openedAt, setOpenedAt] = useState<string | null>(null);
  if (openedAt !== null && openedAt !== pathname) {
    // 遷移で閉じたあとに同じボードへ戻ってきても再び開かないよう、古い値は捨てる
    setOpenedAt(null);
  }
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      // aria-modal を名乗る以上、Tab はドロワー内で循環させる
                ...
                {NAV.map((item, index) => {
                  ...
                  return (
                    <li key={item.href}>
                      <Link
                        href={item.href}
                        // 今いるボードを選んだときは pathname が変わらないので、ここで閉じる
                        onClick={close}
