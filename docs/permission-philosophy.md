...
- allow: 提出系 (`git push` / `gh pr create` / gh 読み取り) + 検証系
  ...
  (実測 4 により封じ込め非破壊)
- deny: `gh pr merge` / `gh pr ready` / `git push --force*` (層 3)。
  Skill / rm は deny しない (退化の撤廃)
...
runner は settings を注入しない代わりに、起動時に
`runner/profile-check.ts` がユーザー settings の不変条件
(層 3 deny の存在・sandbox 有効・bypass でない・広すぎる allow がない) を
検証し、満たさなければ HerdrExecutor を組み立てず fail-closed で停止する。
不変条件そのものの退行は `runner/__tests__/profile-check.test.ts` が守る。
